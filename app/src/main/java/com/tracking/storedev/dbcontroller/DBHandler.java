...
public class DBHandler  {
    ...
    public List<Store> getStoreList(String status, String day) {
        ...
        if (status.equals("Target")) {
            ...
            From from = new Select().from(Store.class).where("VisitDays LIKE ?", '%' + str[0] + '%');
            for (int index = 1; index < str.length; index++) {
                from.or("VisitDays LIKE ?", '%' + str[index] + '%');
            }
            storeList = from.execute();
        }
        return storeList;
    }
    ...
    public List<SectionHeader> getOrderByDay(String strDay, String status){
        ...
        listDataHeader.add(new SectionHeader(storeList, status));
        return listDataHeader;
    }

    public List<SectionHeader> getOrderByDays(String days){
        ...
        for(int i = 0; i < strDays.length; i++){
            ...
            List<Store> storeList = new Select().from(Store.class).where("VisitDays LIKE ?", new String[]{'%' + ""+day + '%'}).execute();
            if(storeList.size() != 0)
